...
| 📚 | `lorebook/HxH_UNIFIED_v59.json` | The **current 758-entry HxH UNIFIED v59** book — present day is the long recovery after E.5; Phases F and G are future scaffolding |
| ⚙️ | `preset/ST_Claude_Preset_v9.json` | **The preset to use with v59.** Adds the past/present/future phase gate and the recovery-period rules |
| 📚 | `lorebook/HxH_UNIFIED_v58.json` | The previous **757-entry v58** book (present day was Phase G) |
| ⚙️ | `preset/ST_Claude_Preset_v8.json` | The preset for v58 — keep only if you still run that book |
| 📝 | `docs/JAILBREAK_ANALYSIS_v58.md` | Why v8 exists: a line-by-line audit of the old v7 preset (Thai) |
| 📝 | `docs/V59_DELTA.md` | What v59 changed and the four edits that produced v9 (Thai) |
...
Use **`preset/ST_Claude_Preset_v9.json`** with the v59 book. (`v8` is kept for the older v58 book —
don't mix them: v9 enforces a "future phase" rule that v58 has no concept of.)

The preset exists because v58's compatibility layer hands four jobs to the preset — NPC Engine, World Sim,
...
- **Phase Gate** — replaces the old preset's reference to a "tier gate" that exists in no version of
  the book. One live anchor at a time; a past phase runs only when the scene declares it; a phase
  that has not happened yet is scaffolding the model may never steer toward; no stage-skipping; and
  the more restrained reading wins when two are possible.
...
- **Recovery rules** — while the live phase is a recovery period, nothing is cured in one scene,
  affection does not heal, medication is never a power switch, and when Vina cannot decide, the
  scene defers rather than deciding for her.

Full rationale, including the config values that were truncating replies:
[`docs/JAILBREAK_ANALYSIS_v58.md`](docs/JAILBREAK_ANALYSIS_v58.md) and
[`docs/V59_DELTA.md`](docs/V59_DELTA.md).
